...
local function tap_options( event )
	storyboard.gotoScene( "lua.scenes.options", {effect="slideLeft", time=400} )
end 

local function tap_help( event )
	storyboard.gotoScene( "lua.scenes.help", {effect="slideLeft", time=400} )
end 

local function tap_credits( event )
	storyboard.gotoScene( "lua.scenes.info", {effect="slideLeft", time=400} )
end 

local function tap_play( event )
	storyboard.gotoScene( "lua.scenes.level", {effect="slideLeft", time=400} )
end 
...
function scene:createScene( event )
	...
	title_1 = display.newImage( sheet, 14 )
	group:insert( title_1 )
	
	title_2 = display.newImage( sheet, 15 )
	group:insert( title_2 )
	
	options = widget.newButton( { 	sheet=sheet,
									defaultFrame=10,
									overFrame=11,
	...
	help = widget.newButton( { 	sheet=sheet,
								defaultFrame=5,
								overFrame=6,
								onRelease=tap_help} )
...
info = widget.newButton( { 	sheet=sheet,
							defaultFrame=3,
							overFrame=4,
	...
	play = widget.newButton( {	sheet=sheet,
								defaultFrame=12,
								overFrame=13,
								onRelease=tap_play} )
